...
while True: 
    ...
    if vsys_voltage>10:
        ...
        percentage=int(int(((round(vsys_voltage,3)-2)/2.5)*100)/5)*5
    else:
        percentage=int(int(((round(vsys_voltage,3)-2)/1.3)*100)/5)*5
    pgb.fill(black)
    ...
    for i in range(2):
        for f in range(2):
            for x in range(8):
                pgb.create_text(str(randint(100000000, 999999999)),10+(i*150),(10*x)+(160*f),white)
