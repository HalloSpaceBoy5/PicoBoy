...
rename("/main.py", "/Flappy Bird/Flappy Bird.py")
...
while True:
    if pgb.button_Home():
        ...
        machine.reset()
        break
    pgb.load_image("/Flappy Bird/flappybird_title.bin")
    pgb.show()
    if pgb.button_start():
            x=open("/Flappy Bird/highscoresFlappy Bird.sc", "r")
            scores=x.read()
            x.close()
            del x
            scores=scores.split("\n")
...
while True:
    ...
    pgb.sound(0)
    sleep(0.1)
    with open("/Flappy Bird/highscoresFlappy Bird.sc", "r") as s:
        scores=s.read().split("\n")
    ...
    with open("/Flappy Bird/highscoresFlappy Bird.sc", "w+") as w:
        w.write("\n".join(newscores[:10]))
    del newscores
    del scores
    while True:
